...
export const validatorRegisterLink = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        const schema = z.object({
            originalUrl: z.string().url().refine((url) => !url.startsWith(`${process.env.HOST}`), {
                message: `La URL no puede comenzar con ${process.env.HOST}`,
            }),
            description: z.string().min(0).max(250).optional(),
            password: z.string().min(4).max(8).optional(),
            shortUrl: z.string().min(6).max(250).optional().refine((url) => url === undefined || /^[a-zA-Z0-9-_]+$/.test(url), {
                message: "La URL corta solo debe contener caracteres alfanuméricos o guiones",
            })
